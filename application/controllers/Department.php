...
public function newdepartment()
  {
  $this->form_validation->set_rules('dptName', 'dptName', 'trim|required');
  if($this->form_validation->run() == TRUE)
    {
    $this->load->model('department_model');
    $result = $this->department_model->verify_and_save();
    echo $result;
    } 
  else 
    {
...
public function ajax_list()
  ...
  foreach ($list as $category)
    ...
    if($category->status == 1)
      { 
      $str = "<span onclick='update_status(".$category->dptid.",0)' id='span_".$category->dptid."'  class='label label-success' style='cursor:pointer'>Active </span>";
      }
    else
      { 
      $str = "<span onclick='update_status(".$category->dptid.",1)' id='span_".$category->dptid."'  class='label label-danger' style='cursor:pointer'> Inactive </span>";
      }
    $row[] = $str;      
    $str2 = '<div class="btn-group" title="View Account">
        <a class="btn btn-primary btn-o dropdown-toggle" data-toggle="dropdown" href="#">Action <span class="caret"></span></a>
          <ul role="menu" class="dropdown-menu dropdown-light pull-right">';
            ...
            $str2.='<li><a title="Edit Record ?" href="'.base_url().'Department/update/'.$category->dptid.'"><i class="fa fa-fw fa-edit text-blue"></i>Edit</a></li>';

            //if($this->permissions('items_category_delete'))
            $str2.='<li><a style="cursor:pointer" title="Delete Record ?" onclick="delete_department('.$category->dptid.')"><i class="fa fa-fw fa-trash text-red"></i>Delete</a></li></ul></div>';      

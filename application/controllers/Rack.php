...
public function __construct(){
  parent::__construct();
  $this->load_global();
  $this->load->model('rack_model','department');
}
...
public function add()
  ...
  $data=$this->data;
  $data['page_title'] = 'Rack';
  $this->load->view('rack', $data);
}

public function newdepartment()
  {
  $this->form_validation->set_rules('rkName', 'rkName', 'trim|required');
  if($this->form_validation->run() == TRUE)
    {
    $this->load->model('rack_model');
    $result = $this->rack_model->verify_and_save();
    echo $result;
    } 
  else 
    {
    echo "Please Enter Rack name.";
    }
}
...
public function view()
  ...
  $data = $this->data;
  $data['page_title'] = 'Rack List';
  $this->load->view('rack_view', $data);
}
...
public function ajax_list()
  ...
  foreach ($list as $category)
    ...
    $no++;
    $row = array();
    $row[] = '<input type="checkbox" name="checkbox[]" value='.$category->rkid.' class="checkbox column_checkbox" >';
      
    $row[] = $category->rkName;
    $row[] = $category->rkDetails;

    if($category->status == 1)
      { 
      $str = "<span onclick='update_status(".$category->rkid.",0)' id='span_".$category->rkid."'  class='label label-success' style='cursor:pointer'>Active </span>";
      }
    else
      { 
      $str = "<span onclick='update_status(".$category->rkid.",1)' id='span_".$category->rkid."'  class='label label-danger' style='cursor:pointer'> Inactive </span>";
      }
    $row[] = $str;      
    $str2 = '<div class="btn-group" title="View Account">
        <a class="btn btn-primary btn-o dropdown-toggle" data-toggle="dropdown" href="#">Action <span class="caret"></span></a>
          <ul role="menu" class="dropdown-menu dropdown-light pull-right">';
            ...
            $str2.='<li><a title="Edit Record ?" href="'.base_url().'Rack/update/'.$category->rkid.'"><i class="fa fa-fw fa-edit text-blue"></i>Edit</a></li>';

            //if($this->permissions('items_category_delete'))
            $str2.='<li><a style="cursor:pointer" title="Delete Record ?" onclick="delete_department('.$category->rkid.')"><i class="fa fa-fw fa-trash text-red"></i>Delete</a></li></ul></div>';      
...
public function add_department_modal()
  ...
  if($this->form_validation->run() == TRUE)
    ...
    $res = array();
    $query = $this->db->select("rkid,rkName")
                    ->where('store_id',get_current_store_id())
                    ->from('db_rack')
                    ->order_by('rkid','desc')
                    ->limit(1)->get();
    $res['id'] = $query->row()->rkid;
    $res['category'] = $query->row()->rkName;
    $res['result'] = $result;

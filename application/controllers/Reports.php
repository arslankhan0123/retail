...
public function customer_statements()
  {
  //$this->permission_check('customer_statement');
  $data = $this->data;
  $data['page_title'] = 'Customer Statement';
  //var_dump($_GET['from_date']); exit();
  if(isset($_GET['customer_id']))
    {
    $sdate = date("Y-m-d", strtotime($_GET['from_date']));
    $edate = date("Y-m-d", strtotime($_GET['to_date']));
    ...
    $data['customer'] = $this->reports->sales_cust_data($custid);
    
    $data['sale'] = $this->reports->sales_dcust_ledger_data($custid,$sdate,$edate);
    $data['payment'] = $this->reports->spayment_dcust_ledger_data($custid,$sdate,$edate);
    $data['voucher'] = $this->reports->voucher_dcust_ledger_data($custid,$sdate,$edate);
  	}
  //var_dump($data['sale']); exit();
  $this->load->view('customer_statement',$data);
}
...
public function supplier_statements()
  ...
  //$this->permission_check('customer_statement');
  $data = $this->data;
  $data['page_title'] = 'Supplier Statement';
  //var_dump($_GET['from_date']); exit();
  if(isset($_GET['supid']))
    {
    $sdate = date("Y-m-d", strtotime($_GET['from_date']));
    ...
    $data['purchase'] = $this->reports->purchase_supplier_ledger_data($supid,$sdate,$edate);
    $data['payment'] = $this->reports->ppayment_supplier_ledger_data($supid,$sdate,$edate);
    $data['voucher'] = $this->reports->voucher_supplier_ledger_data($supid,$sdate,$edate);
  	}
  //var_dump($data['sale']); exit();
  $this->load->view('supplier_statements',$data);
}
	public function pdf_customer_statement()
	{
		if ($this->input->is_ajax_request())
		{
			$customer_id = $_POST['selectedCustomer'];
			$sfdate = $_POST['selectedStartDate'];
			$efdate = $_POST['selectedEndDate'];
			//die(print($customer_id));
		...
		if (isset($customer_id)) {
			$sdate = date("Y-m-d", strtotime($sfdate));
			$edate = date("Y-m-d", strtotime($efdate));
			$custid = $customer_id;
			//var_dump($sdate);var_dump($edate);var_dump($custid); exit();
			$data['sdate'] = $sdate;
			$data['edate'] = $edate;
		...
		//var_dump($data['sale']); exit();
		$this->load->view('customer_statement_pdf', $data);
		...
		// Render the HTML as PDF
			...
			$pdf = $dompdf->output();
			define('UPLOAD_DIR', './uploads/reports/');
			$repName = 'customer_statement';
			@file_put_contents(UPLOAD_DIR . $repName . ".pdf", $pdf);
			$upload_path = 'uploads/reports/';
			// echo json_encode(base_url() . $upload_path . "customer_statement.pdf");
			$data['report_url'] = base_url() . $upload_path . "customer_statement.pdf";
			die(json_encode($data));
	}
	// public function pdf_customer_statement_v1
	// ()
	// {
	// 	define('UPLOAD_DIR', './uploads/reports/');
	// 	//die(print("nazmul"));
	// 	if (!empty($_POST['pdf'])) {
	// 		$html = urldecode($_POST['pdf']);
	// 		@file_put_contents("texto.html", $html);
	// 		$name = str_replace(" ", "+", $_POST['repName']);
	// 		$nombre = $name;
	...
	// 		$upload_path = 'uploads/reports/';
	// 		echo json_encode(base_url() . $upload_path . $nombre . ".pdf");
	// 		//}
	// 	}
	// }

	public function pdf_customer_statement_test()
	{
		
		// $data['report_url'] = base_url() . $upload_path . "customer_statement.pdf";
		$data['report_url'] = "test";
		echo json_encode($data);
	}
}

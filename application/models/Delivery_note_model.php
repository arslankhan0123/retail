...
class Delivery_note_model extends CI_Model {
	...
	//Datatable start
	var $table = 'db_deliverynote as a';
	    ...
	    //}
	      if(!is_admin()){
	      	if($this->session->userdata('role_id')!='2'){
	      		if(!permissions('show_all_users_deliverynotes')){
	      			$this->db->where("upper(a.created_by)",strtoupper($this->session->userdata('inv_username')));
	      		}
	      	}
	      }
	     $deliverynote_from_date = $this->input->post('deliverynote_from_date');
	     $deliverynote_from_date = system_fromatted_date($deliverynote_from_date);
	     $deliverynote_to_date = $this->input->post('deliverynote_to_date');
	     $deliverynote_to_date = system_fromatted_date($deliverynote_to_date);
	     $users = $this->input->post('users');
	     if($users && !empty($users)){
	     	$this->db->where("upper(a.created_by)",strtoupper($users));
	     }
	     if($deliverynote_from_date!='1970-01-01'){
	     	$this->db->where("a.deliverynote_date>=",$deliverynote_from_date);
	     }
	     if($deliverynote_to_date!='1970-01-01'){
	     	$this->db->where("a.deliverynote_date<=",$deliverynote_to_date);
	     }
	...
	public function verify_save_and_update(){
		...
		$this->db->trans_begin();
		$deliverynote_date=system_fromatted_date($deliverynote_date);
	    ...
	    if($command=='save'){//Create deliverynote code unique if first time entry

			$this->db->query("ALTER TABLE db_deliverynote AUTO_INCREMENT = 1");
			
		    $deliverynote_entry = array(
		    				'deliverynote_code' 				=> get_init_code('deliverynote'),
		    				'count_id' 					=> get_count_id('db_deliverynote'),  
		    				...
		    				'deliverynote_note' 				=> $deliverynote_note,
		    				/*System Info*/
		    ...
		    $deliverynote_entry['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();  	
		    $deliverynote_entry['warehouse_id']=(warehouse_module() && warehouse_count()>1) ? $warehouse_id : get_store_warehouse_id();
			$q1 = $this->db->insert('db_deliverynote', $deliverynote_entry);
			$deliverynote_id = $this->db->insert_id();
		}
			...
			$deliverynote_entry['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();  	
			$deliverynote_entry['warehouse_id']=(warehouse_module() && warehouse_count()>1) ? $warehouse_id : get_store_warehouse_id();
			$q1 = $this->db->where('id',$deliverynote_id)->update('db_deliverynote', $deliverynote_entry);

			##############################################START
			//FIND THE PREVIOUSE ITEM LIST ID'S
			$prev_item_ids = $this->db->select("item_id")->from("db_deliverynoteitems")->where("deliverynote_id",$deliverynote_id)->get()->result_array();
			##############################################END

			$q11=$this->db->query("delete from db_deliverynoteitems where deliverynote_id='$deliverynote_id'");
			if(!$q11){
				return "failed";
			}
		}
		...
		for($i=1;$i<=$rowcount;$i++){
			...
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$deliverynote_qty			=$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_3']));
				...
				//$discount_amt 		=($deliverynote_qty * $unit_total_cost)*$discount_input/100;
				...
				$discount_amt_per_unit = $discount_amt/$deliverynote_qty;
				...
				$deliverynoteitems_entry = array(
		    				'deliverynote_id' 			=> $deliverynote_id, 
		    				'deliverynote_status'		=> $deliverynote_status, 
		    				'item_id' 			=> $item_id, 
		    				'description' 		=> $description, 
		    				'deliverynote_qty' 		=> $deliverynote_qty,
		    				...
		    				'status'	 		=> 1,
		    				'seller_points'		=> get_seller_points($item_id) * $deliverynote_qty,

		    			);
				
				$deliverynoteitems_entry['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();  	
				$q2 = $this->db->insert('db_deliverynoteitems', $deliverynoteitems_entry);
		...
		if(isset($send_sms) && $customer_id!=1){
			if(send_sms_using_template($deliverynote_id,1)==true){
		...
		return "success<<<###>>>$deliverynote_id";
	...
	//Get deliverynote_details
	public function get_details($id,$data){
		//Validate This deliverynote already exist or not
		$query=$this->db->query("select * from db_deliverynote where upper(id)=upper('$id')");
		if($query->num_rows()==0){
			show_404();exit;
		}
		else{
			...
			$data['deliverynote_price']=$query->deliverynote_price;
			$data['deliverynote_price']=$query->deliverynote_price;
	...
	public function delete_deliverynote($ids){
      	$this->db->trans_begin();
      	
		//find the this item has the Purchase Return 
		$converted_rec = $this->db->select("*")->where("deliverynote_id in($ids)")->get("db_sales");
		if($converted_rec->num_rows()>0){
			$i=1;
			echo "Can't Delete!<br>These Delivery_notes List Have the Sales Records!";
		...
		$q3=$this->db->delete("db_deliverynote");
		#----------------------------------
	...
	public function get_items_info($rowcount,$item_id){
		...
		//Get Customer Price 
		$deliverynote_price = get_price_level_price($customer_id,$res1->sales_price);
		$deliverynote_price = number_format($deliverynote_price,2,'.','');

		
		$item_available_qty = total_available_qty_items_of_warehouse($warehouse_id,null,$res1->id);// $res1->stock;
		
		$item_tax_amt = ($res1->tax_type=='Inclusive') ? calculate_inclusive($deliverynote_price,$q3->tax) :calculate_exclusive($deliverynote_price,$q3->tax);
		...
		$info = array(
							...
							'item_deliverynote_price' 			=> $deliverynote_price, 
							'item_tax_name' 			=> $q3->tax_name, 
							'item_deliverynote_qty' 			=> 1, 
							'item_tax_id' 				=> $q3->id, 
							'item_tax' 					=> $q3->tax, 
	...
	public function return_deliverynote_list($deliverynote_id){
		...
		foreach ($q1->result() as $res1) {
			...
			$info = array(
							...
							'item_deliverynote_price' 			=> $res1->price_per_unit, 
							'item_tax_name' 			=> $q3->tax_name, 
							'item_deliverynote_qty' 			=> $res1->deliverynote_qty, 
	...
	public function return_row_with_data($rowcount,$info){
		extract($info);
		$item_amount = ($item_deliverynote_price * $item_deliverynote_qty) + $item_tax_amt;
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               ...
               <td id="td_<?=$rowcount;?>_1">
                  ...
                  <a id="td_data_<?=$rowcount;?>_1" href="javascript:void()" onclick="show_deliverynote_item_modal(<?=$rowcount;?>)" title=""><?=$item_name;?></a> 
                  		<i onclick="show_deliverynote_item_modal(<?=$rowcount;?>)" class="fa fa-edit pointer"></i>
                  	</label>
               </td>
               ...
               <td id="td_<?=$rowcount;?>_3">
                  <div class="input-group ">
                     ...
                     <input typ="text" value="<?=format_qty($item_deliverynote_qty);?>" class="form-control no-padding text-center" onkeyup="calculate_tax(<?=$rowcount;?>)" id="td_data_<?=$rowcount;?>_3" name="td_data_<?=$rowcount;?>_3">
                     <span class="input-group-btn">
               ...
               <td id="td_<?=$rowcount;?>_10"><input type="text" name="td_data_<?=$rowcount;?>_10" id="td_data_<?=$rowcount;?>_10" class="form-control text-right no-padding only_currency text-center" onkeyup="calculate_tax(<?=$rowcount;?>)" value="<?=store_number_format($item_deliverynote_price,0);?>"></td>

               <!-- Discount -->
               <td id="td_<?=$rowcount;?>_8">
                  <input type="text" data-toggle="tooltip" title="Click to Change" name="td_data_<?=$rowcount;?>_8" id="td_data_<?=$rowcount;?>_8" class="pointer form-control text-right no-padding only_currency text-center item_discount" value="<?=store_number_format($item_discount,0);?>" onclick="show_deliverynote_item_modal(<?=$rowcount;?>)" readonly>
               </td>
               ...
               <td id="td_<?=$rowcount;?>_12">
                  ...
                  <a id="td_data_<?=$rowcount;?>_12" href="javascript:void()" data-toggle="tooltip" title='Click to Change' onclick="show_deliverynote_item_modal(<?=$rowcount;?>)" title=""><?=$item_tax_name ;?></a>
                  	</label>
               </td>
               ...
               <input type="hidden" id="td_data_<?=$rowcount;?>_4" name="td_data_<?=$rowcount;?>_4" value="<?=$item_deliverynote_price;?>">

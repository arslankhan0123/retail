...
//             }
	...
	public function get_details($id,$data){
		...
		if($query->num_rows()==0){
			...
			$data['maximum_qty']=$query->maximum_qty;
			$data['minimum_qty']=$query->minimum_qty;
			$data['reorder_qty']=$query->reorder_qty;
			...
			$data['opening_stock']=$query->opening_stock;
			...
			$data['scatid']=$query->scatid;
			$data['clid']=$query->clid;
			$data['szid']=$query->szid;
			$data['rkid']=$query->rkid;
			$data['bnid']=$query->bnid;
			$data['bsid']=$query->bsid;
...
public function add_opening_stock(array $info)
  ...
  $openstock = $this->db->select('*')
              ...
              ->where('adjustment_qty',$info['opening_stock'])
              ->get()
              ->row();
 // var_dump($info['store_id']);
 // var_dump($info['warehouse_id']);
 // var_dump($info['item_id']);
 // var_dump($info['opening_stock']);
 // var_dump($openstock);exit();
 if($openstock)
    {
    $adjustment_entry = array(
      'store_id'       => $info['store_id'], 
      'warehouse_id'   => $info['warehouse_id'], 
      'adjustment_id'  => $openstock->adjustment_id,
    ...
    $adjustment_entry = array(
        ...
        'status'            => 1,
            ...
            try{
            	if($openstock)
    			{
    			$insert = $this->db->where('adjustment_id',$openstock->adjustment_id)->update('db_stockadjustmentitems', $adjustment_entry);
    			}
    			else{

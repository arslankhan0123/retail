...
class Orders_model extends CI_Model {
	...
	public function verify_save_and_update(){
		...
		validate_package_offers('max_invoices','db_orders');
		//END
	    ...
	    if($command=='save'){//Create order code unique if first time entry
		    
			$this->db->query("ALTER TABLE db_orders AUTO_INCREMENT = 1");
			
		    $order_entry = array(
		    				'order_code' 				=> get_init_code('order'), 
		    				'count_id' 					=> get_count_id('db_orders'), 
		    ...
		    $order_entry['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();  	
		    $order_entry['warehouse_id']=(warehouse_module() && warehouse_count()>1) ? $warehouse_id : get_store_warehouse_id();  	
			$q1 = $this->db->insert('db_orders', $order_entry);
			...
			$order_entry['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();  
			$order_entry['warehouse_id']=(warehouse_module() && warehouse_count()>1) ? $warehouse_id : get_store_warehouse_id();  		
			$q1 = $this->db->where('id',$order_id)->update('db_orders', $order_entry);

			$q11=$this->db->query("delete from db_orderitems where order_id='$order_id'");
			if(!$q11){
				return "failed";
			}
		}
		...
		for($i=1;$i<=$rowcount;$i++){
			if(isset($_REQUEST['tr_item_id_'.$i]) && !empty($_REQUEST['tr_item_id_'.$i])){
				...
				$order_qty			=$this->xss_html_filter(trim($_REQUEST['td_data_'.$i.'_3']));
				...
				$orderitems_entry['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();  	
				
				$q2 = $this->db->insert('db_orderitems', $orderitems_entry);
				
				// STOCK UPDATE REMOVED FOR ORDERS (per user requirement)
			}
		}//for end
		...
		if($amount>0 && !empty($payment_type)){
			...
			if($command=='update'){
				$tot_payment = $this->db->select('coalesce(sum(payment),0) as payment')->where('order_id',$order_id)->get('db_orderpayments')->row()->payment;
			...
			$payment_code=get_init_code('order_payment');
			$orderpayments_entry = array(
					'payment_code' 		=> $payment_code,
		    		'count_id'	  		=> get_count_id('db_orderpayments'),
					'order_id' 			=> $order_id, 
			...
			$orderpayments_entry['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();  	
			$q3 = $this->db->insert('db_orderpayments', $orderpayments_entry);

			if(!empty($account_id)){
				// ACCOUNT INSERT FOR ORDER PAYMENTS (without affecting inventory stock)
		...
		$q10=$this->update_order_payment_status($order_id);
		if($q10!=1){
			return "failed";
		}
	...
	public function delete_payment($payment_id){
		...
		$reset_accounts = $this->db->select("debit_account_id,credit_account_id")
									->where("ref_purchasepayments_id in ($payment_id)") // Or order payment mapping? Let's check ac_transactions
									->group_by("debit_account_id,credit_account_id")
									->get("ac_transactions");

		$order_id = $this->db->query("select order_id from db_orderpayments where id=$payment_id")->row()->order_id;

		$q1=$this->db->query("delete from db_orderpayments where id='$payment_id'");
		$q2=$this->update_order_payment_status($order_id);
	...
	public function update_order_payment_status_by_order_id($order_id){
		$q8=$this->db->query("select COALESCE(SUM(payment),0) as payment from db_orderpayments where order_id='$order_id'");
		$sum_of_payments=$q8->row()->payment;

		$q9=$this->db->query("select coalesce(grand_total,0) as total from db_orders where id='$order_id'");
		...
		$q7=$this->db->query("update db_orders set 
							payment_status='$payment_status',
							paid_amount=$sum_of_payments 
							where id='$order_id'");
		
		// Order payments do not update supplier purchase due totals, so we do not run supplier update queries here.
		if(!$q7)
		{
			return false;
		}
	...
	function update_order_payment_status($order_id=null){
		if(empty($order_id)){
			return true;
		}
		else{
			if(!$this->update_order_payment_status_by_order_id($order_id)){
				return false;
			}
			return true;
		}
	...
	public function delete_purchase($ids){ // Delete order
      	$this->db->trans_begin();

      	//ACCOUNT RESET
		$reset_accounts = $this->db->select("debit_account_id,credit_account_id")
									->where("ref_purchasepayments_id in ($ids)") // Check order payment mapping
									->group_by("debit_account_id,credit_account_id")
									->get("ac_transactions");
		...
		$this->db->where("order_id in ($ids)");
		if(!is_admin()){
			$this->db->where("store_id",get_current_store_id());
		}
		$q5=$this->db->delete("db_orderpayments");

		$this->db->where("order_id in ($ids)");
		if(!is_admin()){
			$this->db->where("store_id",get_current_store_id());
		}
		$q7=$this->db->delete("db_orderitems");

		// Skip stock updates on deletion as well
	...
	public function return_purchase_list($order_id){
		$q1=$this->db->select('*')->from('db_orderitems')->where("order_id=$order_id")->get();
		$rowcount =1;
		$result = "";
		foreach ($q1->result() as $res1) {
			...
			$info = array(
							...
							'item_tax_name' 			=> $q3->tax_name, 
							'item_purchase_qty' 		=> $res1->order_qty, 
							'item_tax_id' 				=> $res1->tax_id, 
							'item_tax' 					=> $q3->tax, 
							'item_tax_type' 			=> $res1->tax_type, 
	...
	public function show_pay_now_modal($order_id){
		$q1=$this->db->query("select * from db_orders where id=$order_id");
		$res1=$q1->row();
	    ...
	    $order_date=$res1->order_date;
	    $reference_no=$res1->reference_no;
	    $order_code=$res1->order_code;
	    $order_note=$res1->order_note;
		  ...
		  <div class="modal-dialog ">
		    ...
		    <div class="row">
		      <div class="col-md-12">
		      	<div class="row invoice-info">
			        ...
			        <div class="col-sm-4 invoice-col">
			          Order Details:
			          <address>
			            <b>Invoice #<?php echo  $order_code; ?></b><br>
			            <b><?= $this->lang->line('date'); ?> :<?php echo  show_date($order_date); ?></b><br>
		      ...
		      <div class="modal-footer">
		        ...
		        <button type="button" onclick="save_payment(<?=$order_id;?>)" class="btn bg-green btn-lg place_order btn-lg payment_save">Save<i class="fa  fa-check "></i></button>
		      </div>
		    </div>
		    <!-- /.modal-content -->
		  </div>
	...
	public function save_payment(){
		...
		if($amount>0 && !empty($payment_type)){
			...
			$orderpayments_entry['store_id']=$this->db->select("store_id")->where('id',$order_id)->get('db_orders')->row()->store_id;
			$q3 = $this->db->insert('db_orderpayments', $orderpayments_entry);
			if(!empty($account_id)){
				$insert_bit = insert_account_transaction(array(
															'transaction_type'  	=> 'ORDER PAYMENT',
	...
	public function view_payments_modal($order_id){
		$q1=$this->db->query("select * from db_orders where id=$order_id");
		$res1=$q1->row();
		$supplier_id = $res1->supplier_id;
		$q2=$this->db->query("select * from db_suppliers where id=$supplier_id");
		      	...
		      	<div class="row invoice-info">
			        ...
			        <div class="col-sm-4 invoice-col">
			         Order Details:
			          <address>
			            <b>Invoice #<?php echo  $order_code; ?></b><br>
			            <b><?= $this->lang->line('date'); ?> :<?=  show_date($order_date); ?></b><br>
			            <b><?= $this->lang->line('grand_total'); ?> :<?php echo $grand_total; ?></b><br>
			          </address>
			        </div>
			        <!-- /.col -->
		              ...
		              <div class="row">
		         		<div class="col-md-12">
		                      <table class="table table-bordered">
                                ...
                                <tbody>
                                	<?php
                                	$q1=$this->db->query("select * from db_orderpayments where order_id=$order_id");
									$i=1;
									if($q1->num_rows()>0){
										foreach ($q1->result() as $res1) {
											echo "<tr>";

...
public function verify_and_save()
  ...
  if($query->num_rows()>0)
    ...
    $q1 = $this->db->insert('db_subcategory', $info);
    if($q1)
      {
...
public function update_category()
  ...
  if($query->num_rows()>0)
    ...
    $info['store_id']=(store_module() && is_admin()) ? $store_id : get_current_store_id();
    $q1 = $this->db->where('scatid',$q_id)->update('db_subcategory', $info);
    if($q1)
      {
...
public function update_status($id,$status)
  {
  if(set_status_of_table($id,$status,'db_subcategory'))
    {
    echo "success";
    }
  else
...
public function delete_categories_from_table($id)
  {
  $tot = $this->db->query('SELECT COUNT(*) AS tot,b.scatName FROM db_items a,`db_subcategory` b WHERE b.scatid=a.`scatid` AND a.scatid IN ('.$id.') GROUP BY a.scatid');
  ...
  if($tot->num_rows() > 0)
    ...
    $query1 = $this->db->delete("db_subcategory");
    if($query1)
      {
      echo "success";
      }

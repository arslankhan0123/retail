...
$('#save,#update').on("click",function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					//if(confirm("Do You Wants to Save Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'Clcategory/newcolor',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
						success: function(result){
							...
							if(result=="success")
							{
								...
								window.location=base_url+"Clcategory/view";
								return;
							}
	...
	else if(this_id=="update")  //Save start
    {
					...
					//if(confirm("Do You Wants to Update Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'Clcategory/update_color',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
{
	var base_url=$("#base_url").val();
	$.post(base_url+"Clcategory/update_status",{id:id,status:status},function(result){
		if(result=="success")
				{
...
{
	var base_url=$("#base_url").val();
	//alert(base_url); alert(dptid);//return;
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post(base_url+"Clcategory/delete_color",{id:id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
...
function multi_delete(){
    ...
    var this_id=this.id;
		...
		if(confirm("Are you sure ?")){
			...
			$.ajax({
			type: 'POST',
			url: base_url+'Clcategory/multi_delete',
			data: data,
			cache: false,
			contentType: false,
			processData: false,

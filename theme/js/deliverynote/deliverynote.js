...
$('#save,#update').on("click",function (e) {
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    check_field("deliverynote_date");
    check_field("deliverynote_status");
	...
	//Atleast one record must be added in deliverynote table 
    ...
    var this_id=this.id;
			...
			//if(confirm("Do You Wants to Save Record ?")){
				e.preventDefault();
				data = new FormData($('#deliverynote-form')[0]);//form name
        /*Check XSS Code*/
        ...
        $("#"+this_id).attr('disabled',true);  //Enable Save or Update button
				$.ajax({
				type: 'POST',
				url: base_url+'Delivery_note/deliverynote_save_and_update?command='+this_id+'&rowcount='+rowcount+'&tot_subtotal_amt='+tot_subtotal_amt+'&tot_discount_to_all_amt='+tot_discount_to_all_amt+'&tot_round_off_amt='+tot_round_off_amt+'&tot_total_amt='+tot_total_amt+"&other_charges_amt="+other_charges_amt,
				data: data,
				cache: false,
				contentType: false,
				processData: false,
         ...
         // alert(result);return;
				result=result.split("<<<###>>>");
					if(result[0]=="success")
					{
						location.href=base_url+"Delivery_note/invoice/"+result[1];
					}
...
function check_same_item(item_id){
  if($("#deliverynote_table tr").length>1){
...
function add_row(item_id) {
  ...
  $.post(base_url+"Delivery_note/return_row_with_data/"+rowcount+"/"+item_id,{customer_id:customer_id,warehouse_id:warehouse_id},function(result){
        //alert(result);
...
function update_paid_payment_total() {
  ...
  //Delete Record start
function delete_deliverynote(q_id)
{
  var base_url=$("#base_url").val();
   if(confirm("Do You Wants to Delete Record ?")){
    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
    $.post(base_url+"Delivery_note/delete_deliverynote",{q_id:q_id},function(result){
   //alert(result);return;
     if(result=="success")
        {
...
function multi_delete(){
  //var base_url=$("#base_url").val();
    ...
    if(confirm("Are you sure ?")){
      ...
      $.ajax({
      type: 'POST',
      url: base_url+'Delivery_note/multi_delete',
      data: data,
      cache: false,
      contentType: false,
      processData: false,

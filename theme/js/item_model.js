...
$('#save,#update').on("click",function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					//if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#model-form')[0]);//form name
						/*Check XSS Code*/
						...
						$.ajax({
						type: 'POST',
						url: base_url+'ItemModel/new_model',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
             ...
             // alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"ItemModel/view";
								return;
							}
...
{
	var base_url=$("#base_url").val();
	$.post(base_url+"ItemModel/update_status",{id:id,status:status},function(result){
		if(result=="success")
				{
...
//Delete Record start
function delete_model(q_id)
{
	
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("delete_model",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{

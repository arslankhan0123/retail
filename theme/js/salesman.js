...
$('#save,#update').on("click",function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					...
					//if(confirm("Do You Wants to Save Record ?")){
						e.preventDefault();
						data = new FormData($('#salesman-form')[0]);//form name
						/*Check XSS Code*/
						...
						$.ajax({
						type: 'POST',
						url: base_url+'salesman/newsalesman',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
	...
	else if(this_id=="update")  //Update start
    {
					...
					//if(confirm("Do You Wants to Save Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'salesman/update_salesman',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
{
	var base_url=$("#base_url").val();
	$.post(base_url+"salesman/update_status",{id:id,status:status},function(result){
		if(result=="success")
        {
          toastr["success"]("Record Updated Successfully!");
...
//Delete Record start
function delete_salesman(q_id)
{
	var base_url=$("#base_url").val();
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post(base_url+"salesman/delete_salesman",{q_id:q_id},function(result){
  // alert(result);return;
	   if(result=="success")
				{
...
function multi_delete(){
    ...
    var this_id=this.id;
		...
		if(confirm("Are you sure ?")){
			...
			$.ajax({
			type: 'POST',
			url: base_url+'salesman/multi_delete',
			data: data,
			cache: false,
			contentType: false,
			processData: false,
...
function pay_now(salesman_id){

  $.post($("#base_url").val()+'salesman/show_pay_now_modal', {salesman_id: salesman_id}, function(result) {
...
function save_payment(salesman_id){
  var base_url=$("#base_url").val();
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    $.post(base_url+'salesman/save_payment', {account_id:account_id,salesman_id: salesman_id,payment_type:payment_type,amount:amount,payment_date:payment_date,payment_note:payment_note}, function(result) {
      result=result;
  //alert(result);return;
        if(result=="success")
        {
...
function pay_return_due(salesman_id){

  $.post($("#base_url").val()+'salesman/show_pay_return_due_modal', {salesman_id: salesman_id}, function(result) {
...
function save_return_due_payment(salesman_id){
  var base_url=$("#base_url").val();
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
    $(".payment_save").attr('disabled',true);  //Enable Save or Update button
    $.post(base_url+'salesman/save_return_due_payment', {account_id:account_id,salesman_id: salesman_id,payment_type:payment_type,amount:amount,payment_date:payment_date,payment_note:payment_note}, function(result) {
      result=result;
  //alert(result);return;
        if(result=="success")
        {
...
function delete_opening_balance_entry(entry_id){
 if(confirm("Do You Wants to Delete Record ?")){
    var base_url=$("#base_url").val();
    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post(base_url+"salesman/delete_opening_balance_entry",{entry_id:entry_id,salesman_id:$("#q_id").val()},function(result){
   //alert(result);//return;
   result=result;
     if(result=="success")
        { 
  ...
  /*27-06-2020*/
  function view_payments(salesman_id){

  $.post($("#base_url").val()+'salesman/view_payment_list_modal', {salesman_id: salesman_id}, function(result) {

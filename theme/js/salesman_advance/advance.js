...
$('#save,#update').on("click",function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					//if(confirm("Do You Wants to Save Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'salesman_advance/new_advance',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
             ...
             // alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"salesman_advance";
								return;
							}
	...
	else if(this_id=="update")  //Save start
    {
					...
					//if(confirm("Do You Wants to Update Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'salesman_advance/update_advance',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
function delete_advance(q_id)
{
   ...
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post($('#base_url').val()+"salesman_advance/delete_advance",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
...
function multi_delete(){
    ...
    var this_id=this.id;
		...
		if(confirm("Are you sure ?")){
			...
			$.ajax({
			type: 'POST',
			url: base_url+'salesman_advance/multi_delete',
			data: data,
			cache: false,
			contentType: false,
			processData: false,

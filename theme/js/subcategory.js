...
$('#save,#update').on("click",function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					//if(confirm("Do You Wants to Save Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'Subcategory/newcategory',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
	...
	else if(this_id=="update")  //Save start
    {
					...
					//if(confirm("Do You Wants to Update Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'Subcategory/update_category',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
{
	var base_url=$("#base_url").val();
	$.post(base_url+"Subcategory/update_status",{id:id,status:status},function(result){
		if(result=="success")
				{
...
function delete_category(q_id)
{
   ...
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post(base_url+"Subcategory/delete_category",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
...
function multi_delete(){
    ...
    var this_id=this.id;
		...
		if(confirm("Are you sure ?")){
			...
			$.ajax({
			type: 'POST',
			url: base_url+'Subcategory/multi_delete',
			data: data,
			cache: false,
			contentType: false,
			processData: false,
